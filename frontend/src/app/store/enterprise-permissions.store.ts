...
interface EnterprisePermissionsState {
  ...
  updateUser: (userId: string, updates: Partial<EnterpriseUser>) => Promise<EnterpriseUser>;
  ...
  createPermissionRule: (ruleData: PermissionRuleForm) => Promise<PermissionRule>;
  ...
  removeResourcePermission: (resourceId: string, permissionId: string) => Promise<void>;
  getResourcePermissions: (resourceId: string) => ResourcePermission | undefined;
  ...
  removeFolderPermission: (folderId: string, permissionId: string) => Promise<void>;
  ...
  createPermissionMatrix: (matrixData: Partial<PermissionMatrix>) => Promise<PermissionMatrix>;
  updatePermissionMatrix: (matrixId: string, updates: Partial<PermissionMatrix>) => Promise<PermissionMatrix>;
  ...
  evaluatePermission: (userId: string, resource: string, action: string, context?: any) => Promise<PermissionEvaluation>;
  ...
  logPermissionChange: (change: Omit<PermissionChangeLog, 'id' | 'timestamp'>) => Promise<void>;
  logPermissionAudit: (audit: Omit<PermissionAuditLog, 'id' | 'timestamp'>) => Promise<void>;
  ...
  importPermissions: (data: string, format: 'json' | 'csv' | 'xlsx') => Promise<void>;
  resetStore: () => void;
...
const mockRoles: CustomRole[] = [
  {
    ...
    isActive: true,
    permissions: [
      { id: 'perm-1', resource: 'users', action: 'manage', scope: 'global', granted: true, priority: 1 },
      { id: 'perm-2', resource: 'roles', action: 'manage', scope: 'global', granted: true, priority: 1 },
      { id: 'perm-3', resource: 'permissions', action: 'manage', scope: 'global', granted: true, priority: 1 },
      { id: 'perm-4', resource: 'files', action: 'manage', scope: 'global', granted: true, priority: 1 },
    ],
  ...
  },
  {
    ...
    isActive: true,
    permissions: [
      { id: 'perm-5', resource: 'users', action: 'read', scope: 'organization', granted: true, priority: 2 },
      { id: 'perm-6', resource: 'projects', action: 'manage', scope: 'organization', granted: true, priority: 2 },
      { id: 'perm-7', resource: 'files', action: 'read', scope: 'organization', granted: true, priority: 2 },
    ],
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    createdBy: 'system',
  },
  {
    ...
    isActive: true,
    permissions: [
      { id: 'perm-8', resource: 'projects', action: 'read', scope: 'project', granted: true, priority: 3 },
      { id: 'perm-9', resource: 'files', action: 'read', scope: 'project', granted: true, priority: 3, conditions: [
        { id: 'cond-1', field: 'file_type', operator: 'in', value: ['job_sheets', 'specifications', 'project_photos'] }
      ]},
    ],
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
...
export const useEnterprisePermissionsStore = create<EnterprisePermissionsState>()(
  persist(
    (set, get) => ({
      // Initial State
      roles: mockRoles,
      ...
      updateUser: async (userId: string, updates: Partial<EnterpriseUser>) => {
        set({ isSaving: true, error: null });
        try {
          set(state => ({
          ...
          set(state => ({
            users: state.users.map(user => 
              user.id === userId 
                ? { 
                    ...user, 
                    additionalRoles: user.additionalRoles.filter(id => id !== roleId),
                    updatedAt: new Date().toISOString()
                  }
                : user
            ),
      ...
      addCustomPermission: async (userId: string, permission: any) => {
        ...
        try {
          ...
          set({ 
            error: error instanceof Error ? error.message : 'Failed to add custom permission',
            isSaving: false 
          });
          throw error;
        }
      ...
      removeCustomPermission: async (userId: string, permissionId: string) => {
        set({ isSaving: true, error: null });
        try {
          set(state => ({
            users: state.users.map(user => 
              user.id === userId 
                ? { 
                    ...user, 
                    customPermissions: user.customPermissions.filter(p => p.id !== permissionId),
                    updatedAt: new Date().toISOString()
                  }
                : user
            ),
          ...
          set({ 
            error: error instanceof Error ? error.message : 'Failed to remove custom permission',
            isSaving: false 
          });
          throw error;
        }
      ...
      getUsersByRole: (roleId: string) => {
        return get().users.filter(user => 
          user.primaryRole === roleId || user.additionalRoles.includes(roleId)
        );
      },
      ...
      updatePermissionRule: async (ruleId: string, updates: Partial<PermissionRule>) => {
      ...
      getRulesByResource: (resource: string) => {
        return get().permissionRules.filter(rule => rule.resource === resource);
      },

      // Resource Permission Actions (simplified for now)
      setResourcePermission: async (resourceId: string, permission: any) => {
        // TODO: Implement resource permission setting
        throw new Error('Not implemented yet');
      },

      removeResourcePermission: async (resourceId: string, permissionId: string) => {
        // TODO: Implement resource permission removal
        throw new Error('Not implemented yet');
      },

      getResourcePermissions: (resourceId: string) => {
        return get().resourcePermissions.find(rp => rp.resourceId === resourceId);
      },

      // File/Folder Permission Actions (simplified for now)
      setFilePermission: async (fileId: string, permission: any) => {
      ...
      setFolderPermission: async (folderId: string, permission: any) => {
        // TODO: Implement folder permission setting
        throw new Error('Not implemented yet');
      },

      removeFolderPermission: async (folderId: string, permissionId: string) => {
      ...
      createPermissionMatrix: async (matrixData: Partial<PermissionMatrix>) => {
        // TODO: Implement permission matrix creation
        throw new Error('Not implemented yet');
      },

      updatePermissionMatrix: async (matrixId: string, updates: Partial<PermissionMatrix>) => {
      ...
      evaluatePermission: async (userId: string, resource: string, action: string, context?: any) => {
      ...
      logPermissionChange: async (change: Omit<PermissionChangeLog, 'id' | 'timestamp'>) => {
        // TODO: Implement permission change logging
        throw new Error('Not implemented yet');
      },

      logPermissionAudit: async (audit: Omit<PermissionAuditLog, 'id' | 'timestamp'>) => {
      ...
      getChangeLogs: async (filters?: any) => {
      ...
      setSelectedResource: (resourceId: string | null) => set({ selectedResource: resourceId }),
      setMatrixView: (isMatrixView: boolean) => set({ isMatrixView }),
      setCreatingRole: (isCreating: boolean) => set({ isCreatingRole: isCreating }),
      setCreatingUser: (isCreating: boolean) => set({ isCreatingUser: isCreating }),
      setCreatingRule: (isCreating: boolean) => set({ isCreatingRule: isCreating }),

      // Utility Actions
      getPermissionSummary: () => {
        const state = get();
        const totalPermissions = state.roles.reduce((sum, role) => sum + role.permissions.length, 0);
        const grantedPermissions = state.roles.reduce((sum, role) => 
          sum + role.permissions.filter(p => p.granted).length, 0
        );
      ...
      getUserSummary: () => {
        ...
        return {
          ...
          usersWithCustomPermissions: state.users.filter(user => user.customPermissions.length > 0).length,
          usersWithRestrictions: state.users.filter(user => user.restrictions.length > 0).length,
        };
      },
      ...
      importPermissions: async (data: string, format: 'json' | 'csv' | 'xlsx') => {
