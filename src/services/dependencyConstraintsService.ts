...
class DependencyConstraintsService {
  ...
  async getProjectDependencies(projectId: string): Promise<TaskDependency[]> {
    try {
      ...
      return allDependencies.filter(
        dep => dep.projectId === projectId && (isDemoMode ? dep.demo : true)
      );
    } catch (error) {
    ...
    try {
      ...
      if (isDemoMode) {
        const projectDeps = allDependencies.filter(
          dep => dep.projectId === data.projectId && dep.demo
        );

        if (projectDeps.length >= this.maxDemoConstraints) {
          return {
            success: false,
            error: `Maximum ${this.maxDemoConstraints} constraints allowed in demo mode`,
          };
        }
        ...
        if (data.type && !['FS', 'SS'].includes(data.type)) {
          return {
            success: false,
            error: 'Only FS and SS constraint types allowed in demo mode',
          };
        }
        ...
        if (data.lag && (data.lag < 0 || data.lag > this.maxDemoLag)) {
          return {
            success: false,
            error: `Lag must be between 0 and ${this.maxDemoLag} days in demo mode`,
          };
        }
      }
  ...
  async updateDependency(
    ...
    try {
      ...
      const depIndex = allDependencies.findIndex(
      ...
      if (isDemoMode && !dependency.demo) {
        return {
          success: false,
          error: 'Cannot edit non-demo dependencies in demo mode',
        };
      }
      ...
      if (
        isDemoMode &&
        data.lag !== undefined &&
        (data.lag < 0 || data.lag > this.maxDemoLag)
      ) {
        return {
          success: false,
          error: `Lag must be between 0 and ${this.maxDemoLag} days in demo mode`,
  ...
  async deleteDependency(
    ...
    try {
      ...
      if (isDemoMode && !dependency.demo) {
        return {
          success: false,
          error: 'Cannot delete non-demo dependencies in demo mode',
        };
      }

      const updatedDependencies = allDependencies.filter(
        dep => dep.id !== dependencyId
      );
  ...
  async checkConstraintViolations(
    ...
    for (const dependency of dependencies) {
      ...
      const violation = this.checkSingleConstraint(
        dependency,
        sourceTask,
        targetTask
      );
  ...
  private checkSingleConstraint(
    ...
    switch (dependency.type) {
      ...
      case 'SS': // Start → Start
        const requiredStartSS = new Date(
          sourceTask.startDate.getTime() + lagMs
        );
