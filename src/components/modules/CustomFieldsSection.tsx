...
const CustomFieldsSection: React.FC<CustomFieldsSectionProps> = ({
    ...
    const loadCustomFields = async () => {
      try {
        ...
        if (taskId) {
          ...
          const fieldValues =
            await programmeCustomFieldsService.getTaskCustomFieldValues(taskId);
          setCustomFields(fieldValues);
        } else {
          // Create mode: load field definitions only
          const projectFields =
            await programmeCustomFieldsService.getVisibleProjectCustomFields(
              projectId
            );
          const fieldValues: CustomFieldWithValue[] = projectFields.map(
            field => ({
              ...field,
              value: undefined,
            })
  ...
  const handleFieldChange = async (fieldId: string, value: any) => {
    try {
      ...
      if (taskId) {
        const result =
          await programmeCustomFieldsService.saveTaskCustomFieldValue(
            taskId,
            fieldId,
            value
          );
        if (!result.success) {
          setErrors(prev => ({
            ...prev,
            [fieldId]: result.error || 'Failed to save field',
          }));
        }
      }
        ...
        <div className='text-center py-8 text-gray-500 dark:text-gray-400'>
          ...
          {!isDemoMode && canAccess('programme.fields.manage') && (
            <p className='text-sm mt-2'>
              Contact your administrator to add custom fields
            </p>
          )}
        </div>
      </div>
