...
const CustomFieldsManagerModal: React.FC<CustomFieldsManagerModalProps> = ({
  ...
  const [editingField, setEditingField] = useState<ProgrammeCustomField | null>(
    null
  );
  ...
  const loadCustomFields = async () => {
    try {
      setLoading(true);
      const fields =
        await programmeCustomFieldsService.getProjectCustomFields(projectId);
  ...
  const handleUpdateField = async () => {
    ...
    try {
      ...
      const result = await programmeCustomFieldsService.updateCustomField(
        editingField.id,
        {
          label: formData.label,
          type: formData.type,
  ...
  const handleDeleteField = async (fieldId: string) => {
    ...
    if (
      !confirm(
        'Are you sure you want to delete this custom field? This will also delete all values for this field.'
      )
    ) {
      return;
    }
    ...
    try {
      setLoading(true);
      const result =
        await programmeCustomFieldsService.deleteCustomField(fieldId);
                    ...
                    onChange={e =>
                      setFormData(prev => ({
                        ...
                        type: e.target.value as
                          ...
                          | 'dropdown',
                        options:
                          e.target.value === 'dropdown' ? prev.options : [],
                      }))
                    }
                    className='w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:text-white'
                  ...
                  <label className='flex items-center'>
                    <input
                      ...
                      onChange={e =>
                        setFormData(prev => ({
                          ...prev,
                          isVisibleInGrid: e.target.checked,
                        }))
                      }
                      className='rounded border-gray-300 text-blue-600 focus:ring-blue-500'
                      disabled={!canManage || isDemoMode}
                  ...
                  <label className='flex items-center'>
                    <input
                      ...
                      onChange={e =>
                        setFormData(prev => ({
                          ...prev,
                          isVisibleInModal: e.target.checked,
                        }))
                      }
                      className='rounded border-gray-300 text-blue-600 focus:ring-blue-500'
                      disabled={!canManage}
