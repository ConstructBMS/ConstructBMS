...
const ConstraintSelector: React.FC<ConstraintSelectorProps> = ({
  ...
  const [selectedType, setSelectedType] = useState<'SNET' | 'FNLT' | 'MSO' | 'MFO' | 'ASAP'>('SNET');
    ...
    const loadConstraint = async () => {
      try {
        ...
        if (taskConstraint) {
          ...
          setSelectedDate(taskConstraint.constraintDate.toISOString().split('T')[0]);
  ...
  const handleSaveConstraint = async () => {
    ...
    try {
      if (selectedType === 'ASAP') {
        ...
        const result = await constraintsService.setConstraint(taskId, selectedType, constraintDate);
  ...
  const getConstraintTypeOptions = () => {
    const options = [
      { value: 'ASAP', label: 'As Soon As Possible', description: 'No specific date constraint' },
      { value: 'SNET', label: 'Start No Earlier Than', description: 'Cannot start before date' },
      { value: 'FNLT', label: 'Finish No Later Than', description: 'Cannot finish after date' },
      { value: 'MSO', label: 'Must Start On', description: 'Must start exactly on date' },
      { value: 'MFO', label: 'Must Finish On', description: 'Must finish exactly on date' }
    ];

    // Demo mode only allows SNET
    if (isDemoMode) {
      return options.filter(option => option.value === 'SNET' || option.value === 'ASAP');
    }
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {getConstraintTypeOptions().find(opt => opt.value === selectedType)?.description}
            </p>
          </div>
