...
import { programmeWorkingCalendarService, type ProgrammeCalendar, type WorkingTime } from '../../services/programmeWorkingCalendarService';
...
const WorkingCalendarDemo: React.FC = () => {
  ...
  const checkWorkingTime = async () => {
    try {
      const timeData = await programmeWorkingCalendarService.isWorkingDay(selectedDate, projectId);
  ...
  const generateTimelineDays = () => {
    ...
    for (let i = 0; i < 21; i++) { // Show 3 weeks
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Working Calendar Demo</h2>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4">
          <div className="flex items-start space-x-2">
            ...
            <div className="text-sm text-yellow-800">
              ...
              <p className="mt-1">
                Working calendar editing is limited. Only 3 custom exceptions allowed, 
                shift hours are fixed to 08:00-16:00.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-900">Working Pattern</h3>
            </div>
            <div className="space-y-2">
              <p className="text-sm text-gray-600">
                <span className="font-medium">Pattern:</span> {programmeWorkingCalendarService.getShiftPatternDisplayName(calendar.workdays)}
              </p>
              <p className="text-sm text-gray-600">
                <span className="font-medium">Hours:</span> {calendar.shiftStart} - {calendar.shiftEnd}
              </p>
              <p className="text-sm text-gray-600">
                <span className="font-medium">Global Holidays:</span> {calendar.useGlobalHolidays ? 'Enabled' : 'Disabled'}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-900">Selected Date</h3>
            </div>
            <div className="space-y-2">
              <p className="text-sm text-gray-600">
                <span className="font-medium">Date:</span> {selectedDate.toLocaleDateString('en-GB', {
                  weekday: 'long',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
              ...
              <p className={`text-sm font-medium ${getWorkingTimeStatus().color}`}>
