...
const BaselineManagerModal: React.FC<BaselineManagerModalProps> = ({
  ...
  const [baselineComparison, setBaselineComparison] =
    useState<BaselineComparison | null>(null);
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [formData, setFormData] = useState<CreateBaselineFormData>({
    name: '',
  });
  ...
  const calculateBaselineComparison = async () => {
    try {
      const comparison = await baselineService.calculateBaselineDeltas(
        projectId,
        currentTasks
      );
            ...
            <div className='mb-6 p-4 bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 rounded-lg'>
              ...
              <div className='space-y-4'>
                ...
                <div className='text-sm text-blue-700 dark:text-blue-300'>
                  <p>
                    This will create a snapshot of {currentTasks.length} current
                    tasks.
                  </p>
                  {isDemoMode && (
              ...
              <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
                ...
                <div className='text-center'>
                  <div className='text-2xl font-bold text-gray-600 dark:text-gray-400'>
                    {baselineComparison.totalDurationChange > 0 ? '+' : ''}
                    {baselineComparison.totalDurationChange}
                  </div>
                  <div className='text-sm text-gray-600 dark:text-gray-400'>
                    Total Drift (days)
                  </div>
