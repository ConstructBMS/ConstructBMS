...
const Contracts: React.FC = () => {
  ...
  useEffect(() => {
    const savedState = localStorage.getItem('contractsStatsExpanded');
    if (savedState !== null) {
  ...
  const toggleStats = () => {
    ...
    localStorage.setItem('contractsStatsExpanded', JSON.stringify(newState));
  };
        ...
        {statsExpanded && (
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-top-2 duration-200'>
            <div className='bg-white rounded-xl border p-6 flex items-center'>
              <FileText className='h-8 w-8 text-constructbms-blue mr-4' />
              <div>
                <p className='text-sm text-gray-500'>Total Contracts</p>
                <p className='text-2xl font-bold'>{demoContracts.length}</p>
              </div>
            </div>
            <div className='bg-white rounded-xl border p-6 flex items-center'>
              ...
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {demoContracts.filter(c => c.status === 'active').length}
                </p>
              </div>
            </div>
            <div className='bg-white rounded-xl border p-6 flex items-center'>
              ...
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {demoContracts.filter(c => c.status === 'pending').length}
                </p>
              </div>
            </div>
          </div>
          ...
          <tbody>
            {demoContracts.map(contract => (
              <tr key={contract.id} className='hover:bg-gray-50'>
                ...
                <td className='px-4 py-2'>{contract.signed || '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
