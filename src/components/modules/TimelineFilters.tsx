...
import {
  ...
  TimelineFilters,
  FilterOption,
} from '../services/timelineFiltersService';
...
const TimelineFilters: React.FC<TimelineFiltersProps> = ({
  ...
  useEffect(() => {
    const loadFilters = async () => {
      try {
        const savedFilters =
          await timelineFiltersService.getFilterPreferences(projectId);
  ...
  const handleFilterChange = async (
    ...
    try {
      ...
      await timelineFiltersService.saveFilterPreferencesDebounced(
        projectId,
        newFilters
      );
    } catch (error) {
                ...
                {selectedCount > 0 && (
                  <button
                    onClick={() => {
                      ...
                      setFilters(newFilters);
                      onFiltersChange(newFilters);
                      timelineFiltersService.saveFilterPreferencesDebounced(
                        projectId,
                        newFilters
                      );
                    }}
      ...
      {hasActiveFilters() && (
        <div className='flex flex-wrap items-center space-x-2'>
          ...
          {filters.assignees.map(assigneeId => {
            const option = filterOptions.assignees.find(
              a => a.id === assigneeId
            );
            return (
              <span
              ...
              >
                ...
                <button
                  onClick={() =>
                    handleFilterChange('assignees', assigneeId, false)
                  }
