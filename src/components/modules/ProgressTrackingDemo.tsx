...
import { taskProgressIntegration, type TaskWithProgress } from '../../services/taskProgressIntegration';
...
const ProgressTrackingDemo: React.FC = () => {
  ...
  const loadDemoData = async () => {
    ...
    try {
      ...
      const demoTasks = await taskProgressIntegration.getTasksWithProgress('demo-project');
      setTasks(demoTasks);
      
      // Load project stats
      const stats = await taskProgressIntegration.getProjectProgressStats('demo-project');
  ...
  const initializeDemoProject = async () => {
    try {
      ...
      const { initializeProgressDemo } = await import('../../scripts/initializeProgressDemo');
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        ...
        <span className="ml-2 text-gray-600">Loading progress tracking demo...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              {showProgressBars ? <EyeSlashIcon className="h-4 w-4 mr-2" /> : <EyeIcon className="h-4 w-4 mr-2" />}
        ...
        {projectStats && (
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            ...
            <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
              <div className="text-2xl font-bold text-gray-600">{projectStats.notStartedTasks}</div>
            ...
            <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg">
              <div className="text-2xl font-bold text-orange-600">{projectStats.demoTasks}</div>
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <div className="flex items-start">
            ...
            <div className="text-sm text-orange-800">
              ...
              <p className="mt-1">
                Progress is limited to 75%, actual dates are disabled, and only 10 tasks can be edited.
                Upgrade to track full progress and actual dates.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4">
          <label className="flex items-center">
            <input
              ...
              className="mr-2"
            />
            <span className="text-sm text-gray-700 dark:text-gray-300">Show Actual Date Markers</span>
          </label>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white">Tasks with Progress</h3>
        </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                    <div className="space-y-1">
                      <div className="flex items-center">
                        ...
                        Start: {task.actualStartDate ? new Date(task.actualStartDate).toLocaleDateString() : 'Not set'}
                      </div>
                      <div className="flex items-center">
                        <CheckCircleIcon className="h-3 w-3 mr-1" />
                        Finish: {task.actualFinishDate ? new Date(task.actualFinishDate).toLocaleDateString() : 'Not set'}
                      </div>
                    </div>
                  </td>
