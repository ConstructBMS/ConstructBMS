...
const HierarchicalTaskTable: React.FC<HierarchicalTaskTableProps> = ({
  ...
  const getVisibleTasks = (taskList: HierarchicalTask[]): HierarchicalTask[] => {
  ...
  const renderTaskRow = (task: HierarchicalTask, index: number) => {
    ...
    const getTaskIcon = () => {
      if (task.type === 'phase') return <FolderIcon className="w-4 h-4 text-blue-500" />;
      if (task.type === 'milestone') return <CheckCircleIcon className="w-4 h-4 text-green-500" />;
    ...
    return (
      ...
      >
        ...
        {/* Status */}
        <td className="px-3 py-2">
          <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(task.statusId)}`}>
