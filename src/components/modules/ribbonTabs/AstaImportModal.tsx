...
import { astaImportExportService, ParsedAstaProgramme, AstaTask } from '../../../services/astaImportExportService';
...
const AstaImportModal: React.FC<AstaImportModalProps> = ({
  ...
  const [parsedData, setParsedData] = useState<ParsedAstaProgramme | null>(null);
  ...
  const handleFileSelect = async (file: File) => {
    ...
    if (!validExtensions.includes(fileExtension)) {
      setError('Please select a valid Asta file (.pp, .xml, .csv, .mpx, or .json)');
      return;
    }
    ...
    try {
      ...
      setError('Failed to parse the selected file. Please ensure it\'s a valid Asta programme file.');
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="mt-3 text-xs text-gray-500 dark:text-gray-400">
                  Supported formats: Asta PowerProject (.pp), XML (.xml), CSV (.csv), MPX (.mpx), JSON (.json)
                </div>
              </div>
            </div>
          )}
          ...
          {parsedData && (
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 text-gray-900 dark:text-white">{selectedFile?.name}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700 dark:text-gray-300">Project Name:</span>
                    <span className="ml-2 text-gray-900 dark:text-white">{parsedData.projectName}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700 dark:text-gray-300">Total Tasks:</span>
                    <span className="ml-2 text-gray-900 dark:text-white">{parsedData.taskCount}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700 dark:text-gray-300">File Size:</span>
                    <span className="ml-2 text-gray-900 dark:text-white">
                      {selectedFile ? `${(selectedFile.size / 1024).toFixed(1)} KB` : 'Unknown'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    {Object.entries(fieldMapping).map(([field, mappedField]) => (
                      <div key={field} className="flex items-center justify-between">
                        <span className="font-medium text-gray-700 dark:text-gray-300">{field}:</span>
                        <span className="text-gray-900 dark:text-white">{mappedField}</span>
                      </div>
                    ))}
                  </div>
                </div>
