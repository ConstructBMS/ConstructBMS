...
const AstaExportModal: React.FC<AstaExportModalProps> = ({
  ...
  const loadExportCount = async () => {
    try {
      const count =
        await astaImportExportService['getDemoExportCount'](projectId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isDemoMode && exportCount >= 3) {
      setError(
        'Demo mode limited to 3 exports per session. Please upgrade for unlimited exports.'
      );
      return;
    }
    ...
    const dateRange =
      settings.dateRange.end.getTime() - settings.dateRange.start.getTime();
    const maxRange = isDemoMode
      ? 7 * 24 * 60 * 60 * 1000
      : 365 * 24 * 60 * 60 * 1000; // 7 days demo, 1 year full

    if (dateRange > maxRange) {
      setError(
        `Date range too large. Maximum allowed: ${isDemoMode ? '7 days' : '1 year'} in ${isDemoMode ? 'demo' : 'full'} mode.`
      );
      return;
    }
            ...
            <div className='mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg'>
              <div className='space-y-2'>
                ...
                <div className='text-sm text-blue-600 dark:text-blue-400 space-y-1'>
                  <div>
                    • Maximum 3 exports per session (used: {exportCount}/3)
                  </div>
            ...
            <div>
              ...
              <div className='space-y-3'>
                {[
                  ...
                  {
                    value: 'xer',
                    label: 'Primavera XER (.xer)',
                    description: 'Most compatible with Asta PowerProject',
                  },
                  {
                    value: 'mpx',
                    label: 'Microsoft Project MPX (.mpx)',
                    description: 'Standard project management format',
                  },
                ].map(format => (
                  <label key={format.value} className='flex items-start'>
                    <input
                      ...
                      onChange={e =>
                        updateSettings({ fileType: e.target.value as any })
                      }
                      disabled={!canEdit}
                      className='mt-1 mr-3'
                    />
                ...
                {[
                  {
                    ...
                    label: 'Include Task Constraints',
                    description: 'Export task start/finish constraints',
                  },
                  {
                    key: 'includeBaselines',
                    label: 'Include Baselines',
                    description: 'Export project baseline data',
                  },
                  {
                    key: 'includeNotes',
                    label: 'Include Notes',
                  ...
                  },
                  {
                    ...
                    description: 'Export resource assignments',
                  },
                  {
                    key: 'includeCalendars',
                    label: 'Include Calendars',
                    description: 'Export working calendars',
                  },
                ].map(option => (
                  <label key={option.key} className='flex items-start'>
                    <input
                      ...
                      checked={
                        settings[
                          option.key as keyof AstaExportSettings
                        ] as boolean
                      }
                      onChange={e =>
                        updateSettings({
