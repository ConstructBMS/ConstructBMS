...
const TaskCalendarsDemo: React.FC = () => {
  ...
  const loadDemoData = async () => {
    try {
      ...
      // Load calendars
      const projectCalendars = await taskCalendarService.getProjectCalendars(demoProjectId);
      ...
      if (projectCalendars.length === 0 && isDemo) {
        ...
        const updatedCalendars = await taskCalendarService.getProjectCalendars(demoProjectId);
  ...
  const getWorkingDaysDisplay = (workingDays: string[]): string => {
    ...
    if (workingDays.length === 5 && workingDays.includes('Mon') && workingDays.includes('Fri')) {
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-lg">
          Manage custom working calendars for tasks, define holidays, and visualize non-working days in the timeline.
        </p>
      </div>
            ...
            <div className="space-y-3">
              ...
              {calendars.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">Click "Manage Calendars" to create your first calendar</p>
                </div>
              ) : (
                <div className="space-y-3">
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <div>
                          ...
                          <p>{calendar.dailyHours.start} - {calendar.dailyHours.end}</p>
                        </div>
                        <div>
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-blue-800 dark:text-blue-300">
                  <li className="flex items-start space-x-2">
                    ...
                    <span>Tasks respect their assigned calendar for duration calculations</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <CheckIcon className="w-4 h-4 mt-0.5 flex-shrink-0" />
                    <span>Non-working days are excluded from task schedules</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <CheckIcon className="w-4 h-4 mt-0.5 flex-shrink-0" />
                    <span>Global calendars are available across all projects</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <CheckIcon className="w-4 h-4 mt-0.5 flex-shrink-0" />
                    <span>Tasks without assigned calendars use the project default</span>
                  </li>
                </ul>
              </div>
            </div>
