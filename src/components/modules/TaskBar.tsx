...
const TaskBar: React.FC<TaskBarProps> = ({
  ...
  const getTaskBarColor = () => {
    const colorInfo = taskBarColorService.getTaskBarColor(
      task,
      taskStatuses,
      projectTags
    );
    ...
    (e: MouseEvent) => {
      ...
      if (isDemoMode) {
        ...
        if (newDuration > maxDays) {
          ...
          newEndDate = new Date(
            newStartDate.getTime() + originalDuration * 24 * 60 * 60 * 1000
          );
        }
      }
        ...
        if (newDuration > maxDuration) {
          if (resizeState.resizeHandle === 'left') {
            newStartDate = new Date(
              newEndDate.getTime() - maxDuration * 24 * 60 * 60 * 1000
            );
          } else {
            newEndDate = new Date(
              newStartDate.getTime() + maxDuration * 24 * 60 * 60 * 1000
            );
          }
        }
      }
  ...
  const getBaselineStyling = () => {
    ...
    const maxDrift = Math.max(
      Math.abs(baselineDelta.startDrift),
      Math.abs(baselineDelta.endDrift)
    );
          ...
          <div className='space-y-2'>
            ...
            {showBaseline && baselineDelta && !isDemoMode && (
              <div className='border-t border-gray-200 dark:border-gray-600 pt-2'>
                ...
                <div className='space-y-1 text-xs'>
                  <div
                    className={`flex justify-between ${baselineService.getDeltaColorClass(baselineDelta.startDrift)}`}
                  >
                    <span>Start Drift:</span>
                    <span className='font-medium'>
                      {baselineService.formatDelta(baselineDelta.startDrift)}
                    </span>
                  </div>
                  <div
                    className={`flex justify-between ${baselineService.getDeltaColorClass(baselineDelta.endDrift)}`}
                  >
                    <span>End Drift:</span>
                    <span className='font-medium'>
                      {baselineService.formatDelta(baselineDelta.endDrift)}
                    </span>
                  </div>
                  <div
                    className={`flex justify-between ${baselineService.getDeltaColorClass(baselineDelta.durationChange)}`}
                  >
                    <span>Duration Change:</span>
                    <span className='font-medium'>
                      {baselineService.formatDelta(
                        baselineDelta.durationChange
                      )}
                    </span>
                  </div>
                </div>
            ...
            <div className='flex items-center space-x-2'>
              ...
              <span className='text-sm text-gray-600 dark:text-gray-300'>
                {taskStatuses.find(s => s.id === task.statusId)?.name ||
                  'Unknown'}
              </span>
            </div>
            ...
            {showCriticalPath && isCritical && (
              <div className='flex items-center space-x-2 text-sm'>
                <FireIcon
                  className={`w-4 h-4 ${isDemoMode ? 'text-red-300' : 'text-red-500'}`}
                />
                <span
                  className={`font-medium ${isDemoMode ? 'text-red-300' : 'text-red-500'}`}
                >
                  {isDemoMode
                    ? 'DEMO CRITICAL PATH'
                    : 'Critical Task - No Slack'}
                </span>
              </div>
            )}
