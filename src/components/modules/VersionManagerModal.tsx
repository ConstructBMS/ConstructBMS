...
const VersionManagerModal: React.FC<VersionManagerModalProps> = ({
  ...
  const [preferences, setPreferences] = useState<VersionPreferences | null>(
    null
  );
  ...
  const [editData, setEditData] = useState<{ label: string; notes: string }>({
    label: '',
    notes: '',
  });
  ...
  const loadPreferences = async () => {
    try {
      const prefs = await programmeVersioningService.getVersionPreferences(
        'current-user',
        projectId
      );
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const result = await programmeVersioningService.updateVersion(
        editingVersion,
        editData
      );
      if (result) {
          ...
          <div className='mb-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg'>
            <div className='flex items-center'>
              ...
              <span className='text-yellow-800 dark:text-yellow-200 text-sm font-medium'>
                Demo Mode: Maximum {demoConfig.maxVersionsPerProject} versions
                allowed. Version restore is disabled.
              </span>
            </div>
          </div>
        )}
