...
const TimelineZoomControls: React.FC<TimelineZoomControlsProps> = ({
  ...
  const [currentSettings, setCurrentSettings] =
    useState<TimelineZoomSettings | null>(null);
  ...
  const handleTimeScaleChange = async (
    ...
    try {
      setLoading(true);
      const result = await timelineZoomService.changeTimeScale(
        projectId,
        scale
      );
  ...
  const handleScrollToDate = async () => {
    ...
    try {
      ...
      const result = await timelineZoomService.scrollToDate(
        projectId,
        targetDate
      );
  ...
  const handleFitToView = async () => {
    ...
    try {
      ...
      const result = await timelineZoomService.fitToView(
        projectId,
        sampleTasks
      );
  ...
  const getZoomLevelDisplay = (): string => {
    ...
    return timelineZoomService.getZoomLevelDisplayName(
      currentSettings.zoomLevel
    );
  };
  ...
  return (
    <div
      className={`flex items-center space-x-2 p-2 bg-white border border-gray-200 rounded-lg shadow-sm ${className}`}
    >
      {/* Zoom In Button */}
      <button
      ...
      <div className='px-3 py-2 bg-gray-100 rounded-md'>
        ...
        <div className='text-xs text-gray-500'>{getScaleDisplay()}</div>
      </div>
