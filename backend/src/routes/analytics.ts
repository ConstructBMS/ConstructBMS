...
router.get('/project-status', async (req: Request, res: Response) => {
  try {
    ...
    const chartData = [
      { label: 'Planning', value: statusCounts.planning || 0, color: '#3b82f6', gradient: 'from-blue-400 to-blue-600' },
      { label: 'In Progress', value: statusCounts.in_progress || 0, color: '#f59e0b', gradient: 'from-yellow-400 to-orange-500' },
      { label: 'Review', value: statusCounts.review || 0, color: '#8b5cf6', gradient: 'from-purple-400 to-purple-600' },
      { label: 'Completed', value: statusCounts.completed || 0, color: '#10b981', gradient: 'from-green-400 to-green-600' }
    ];
...
router.get('/revenue-trend', async (req: Request, res: Response) => {
  try {
    ...
    const monthlyRevenue = data.reduce((acc: Record<string, number>, item) => {
      const month = new Date(item.revenue_date).toLocaleDateString('en-US', { month: 'short' });
...
router.get('/expense-breakdown', async (req: Request, res: Response) => {
  try {
    ...
    if (error) {
      ...
      // Return mock data if table doesn't exist or any other error
      const mockData = [
        { label: 'Materials', value: 18500, color: '#3b82f6', gradient: 'from-blue-400 to-blue-600', icon: '🧱' },
        { label: 'Labor', value: 12300, color: '#f59e0b', gradient: 'from-yellow-400 to-orange-500', icon: '👷' },
        { label: 'Equipment', value: 4200, color: '#8b5cf6', gradient: 'from-purple-400 to-purple-600', icon: '🔧' },
        { label: 'Overhead', value: 3800, color: '#10b981', gradient: 'from-green-400 to-green-600', icon: '🏢' },
        { label: 'Subcontractors', value: 5600, color: '#ef4444', gradient: 'from-red-400 to-red-600', icon: '🤝' }
      ];
      return res.json(mockData);
    }
    ...
    const categoryConfig = {
      materials: { label: 'Materials', color: '#3b82f6', gradient: 'from-blue-400 to-blue-600', icon: '🧱' },
      labor: { label: 'Labor', color: '#f59e0b', gradient: 'from-yellow-400 to-orange-500', icon: '👷' },
      equipment: { label: 'Equipment', color: '#8b5cf6', gradient: 'from-purple-400 to-purple-600', icon: '🔧' },
      overhead: { label: 'Overhead', color: '#10b981', gradient: 'from-green-400 to-green-600', icon: '🏢' },
      subcontractors: { label: 'Subcontractors', color: '#ef4444', gradient: 'from-red-400 to-red-600', icon: '🤝' },
      other: { label: 'Other', color: '#6b7280', gradient: 'from-gray-400 to-gray-600', icon: '📦' }
    };

    const chartData = Object.entries(categoryTotals).map(([category, amount]) => ({
      label: categoryConfig[category as keyof typeof categoryConfig]?.label || category,
      value: amount,
      color: categoryConfig[category as keyof typeof categoryConfig]?.color || '#6b7280',
      gradient: categoryConfig[category as keyof typeof categoryConfig]?.gradient || 'from-gray-400 to-gray-600',
      icon: categoryConfig[category as keyof typeof categoryConfig]?.icon || '📦'
    }));
...
router.get('/cash-flow', async (req: Request, res: Response) => {
  try {
    ...
    const monthlyRevenue = (revenueData || []).reduce((acc: Record<string, number>, item) => {
      const month = new Date(item.revenue_date).toLocaleDateString('en-US', { month: 'short' });
      acc[month] = (acc[month] || 0) + Number(item.amount);
      return acc;
    }, {});
    ...
    const monthlyExpenses = (expenseData || []).reduce((acc: Record<string, number>, item) => {
      const month = new Date(item.expense_date).toLocaleDateString('en-US', { month: 'short' });
      acc[month] = (acc[month] || 0) + Number(item.amount);
      return acc;
    }, {});
...
router.get('/business-overview', async (req: Request, res: Response) => {
  try {
    ...
    if (projectsError || contactsError || tasksError || revenueError) {
      log('Error fetching business overview:', projectsError || contactsError || tasksError || revenueError);
