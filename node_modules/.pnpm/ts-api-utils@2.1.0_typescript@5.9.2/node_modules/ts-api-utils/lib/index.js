...
function isJsxElementOrFragment(node) {
  return (
    node.kind === ts9.SyntaxKind.JsxElement ||
    node.kind === ts9.SyntaxKind.JsxFragment
  );
}
...
function isAssignmentKind(kind) {
  return (
    kind >= ts9.SyntaxKind.FirstAssignment &&
    kind <= ts9.SyntaxKind.LastAssignment
  );
}
...
function isValidPropertyAccess(
  text,
  languageVersion = ts9.ScriptTarget.Latest
) {
  if (text.length === 0) {
    return false;
  }
    ...
    switch (node.parent.kind) {
      ...
      case ts9.SyntaxKind.BinaryExpression:
        return (
          ...
          node.parent.operatorToken.kind === ts9.SyntaxKind.EqualsToken
        );
      case ts9.SyntaxKind.ForOfStatement:
...
var [tsMajor, tsMinor] = ts9.versionMajorMinor
  .split('.')
...
function isAccessExpression(node) {
  return (
    ts9.isPropertyAccessExpression(node) || ts9.isElementAccessExpression(node)
  );
}
...
function isAccessorDeclaration(node) {
  return (
    ts9.isGetAccessorDeclaration(node) || ts9.isSetAccessorDeclaration(node)
  );
}
...
function isAssignmentPattern(node) {
  return (
    ts9.isObjectLiteralExpression(node) || ts9.isArrayLiteralExpression(node)
  );
}
...
function isJSDocComment(node) {
  ...
  if (isTsVersionAtLeast(4, 4)) {
    return (
      ts9.isJSDocLink(node) ||
      ts9.isJSDocLinkCode(node) ||
      ts9.isJSDocLinkPlain(node)
    );
  }
  return false;
}
...
function isObjectBindingOrAssignmentPattern(node) {
  return (
    ts9.isObjectBindingPattern(node) || ts9.isObjectLiteralExpression(node)
  );
}
...
function isPseudoLiteralToken(node) {
  return (
    ts9.isTemplateHead(node) ||
    ts9.isTemplateMiddle(node) ||
    ts9.isTemplateTail(node)
  );
}
...
function isTypeOnlyCompatibleAliasDeclaration(node) {
  ...
  if (
    isTsVersionAtLeast(5, 0) &&
    (ts9.isExportDeclaration(node) || ts9.isNamespaceExport(node))
  ) {
    return true;
  }
  return false;
...
function isTypeReferenceType(node) {
  return (
    ts9.isTypeReferenceNode(node) || ts9.isExpressionWithTypeArguments(node)
  );
}
...
function isSuperElementAccessExpression(node) {
  return (
    ts9.isElementAccessExpression(node) && isSuperExpression(node.expression)
  );
}
function isSuperPropertyAccessExpression(node) {
  return (
    ts9.isPropertyAccessExpression(node) && isSuperExpression(node.expression)
  );
}
...
function isEvolvingArrayType(type) {
  return (
    isObjectType(type) && isObjectFlagSet(type, ts9.ObjectFlags.EvolvingArray)
  );
}
    ...
    if (getPropertyOfType(subType, name) === undefined) {
      const index =
        (isNumericPropertyName(name)
          ...
          : undefined) ??
        typeChecker.getIndexInfoOfType(subType, ts9.IndexKind.String);
      if (index?.isReadonly) {
        if (seenProperty) {
          return true;
        }
  ...
  return !!(
    ...
    symbol.declarations?.some(
      node =>
        ...
        (ts9.isVariableDeclaration(node) &&
          isNodeFlagSet(node.parent, ts9.NodeFlags.Const)) ||
        (ts9.isCallExpression(node) &&
...
function typeIsLiteral(type) {
  if (isTsVersionAtLeast(5, 0)) {
    ...
    return isTypeFlagSet(
      type,
      ts9.TypeFlags.StringLiteral |
        ts9.TypeFlags.NumberLiteral |
        ts9.TypeFlags.BigIntLiteral
    );
  }
}
...
function getPropertyName(propertyName) {
  ...
  return propertyName.kind === ts9.SyntaxKind.PrivateIdentifier
    ? undefined
    : propertyName.text;
}
...
function getUsageDomain(node) {
  ...
  switch (parent.kind) {
    ...
    case ts9.SyntaxKind.TypeReference:
      return identifierToKeywordKind(node) !== ts9.SyntaxKind.ConstKeyword
        ? 2 /* Type */
        : undefined;
    default:
      return 5 /* ValueOrNamespace */;
...
var UsageWalker = class {
  ...
  getUsage(sourceFile) {
      ...
      switch (node.kind) {
        ...
        case ts9.SyntaxKind.TypeParameter:
          this.#scope.addVariable(
            node.name.text,
            node.name,
            node.parent.kind === ts9.SyntaxKind.InferType
              ? 8 /* InferType */
              : 7 /* Type */,
            false,
            2 /* Type */
    ...
    const handleBlockScope = node => {
      if (
        node.kind === ts9.SyntaxKind.CatchClause &&
  ...
  #handleFunctionLikeDeclaration(node, cb, varCb) {
    ...
    const scope = (this.#scope =
      node.kind === ts9.SyntaxKind.FunctionExpression && node.name !== undefined
