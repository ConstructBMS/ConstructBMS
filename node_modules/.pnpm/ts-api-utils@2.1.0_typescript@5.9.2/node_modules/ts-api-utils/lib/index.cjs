...
function forEachComment(node, callback, sourceFile = node.getSourceFile()) {
  ...
  const notJsx =
    sourceFile.languageVariant !== ts9__default.default.LanguageVariant.JSX;
  return forEachToken(
    node,
    token => {
      ...
      if (token.kind !== ts9__default.default.SyntaxKind.JsxText) {
        ts9__default.default.forEachLeadingCommentRange(
          ...
          token.pos === 0
            ? (ts9__default.default.getShebang(fullText) ?? '').length
            : token.pos,
          commentCallback
        );
      }
...
function isValidPropertyAccess(
  text,
  languageVersion = ts9__default.default.ScriptTarget.Latest
) {
  if (text.length === 0) {
    return false;
  }
...
function getAccessKind(node) {
  ...
  switch (parent.kind) {
      ...
      return parent.right === node
        ...
        : !isAssignmentKind(parent.operatorToken.kind)
          ...
          : parent.operatorToken.kind ===
              ts9__default.default.SyntaxKind.EqualsToken
            ? 2 /* Write */
            : 3 /* ReadWrite */;
    ...
    case ts9__default.default.SyntaxKind.ExpressionWithTypeArguments:
      return parent.parent.token ===
        ts9__default.default.SyntaxKind.ExtendsKeyword &&
        parent.parent.parent.kind !==
          ts9__default.default.SyntaxKind.InterfaceDeclaration
        ? 1 /* Read */
        : 0 /* None */;
    ...
    case ts9__default.default.SyntaxKind.PrefixUnaryExpression:
      return parent.operator ===
        ts9__default.default.SyntaxKind.PlusPlusToken ||
...
function isInDestructuringAssignment(node) {
  switch (node.kind) {
    ...
    case ts9__default.default.SyntaxKind.SpreadElement:
      if (
        node.parent.kind !==
        ts9__default.default.SyntaxKind.ArrayLiteralExpression
      ) {
        return false;
      }
      node = node.parent;
...
function hasJSDoc(node) {
  ...
  if (
    isTsVersionAtLeast(4, 4) &&
    ts9__default.default.isClassStaticBlockDeclaration(node)
  ) {
    return true;
  }
  if (
...
function isBindingOrAssignmentElementRestIndicator(node) {
  if (
    ts9__default.default.isSpreadElement(node) ||
    ts9__default.default.isSpreadAssignment(node)
  ) {
    return true;
  }
...
function isEntityNameOrEntityNameExpression(node) {
  return (
    ts9__default.default.isEntityName(node) || isEntityNameExpression(node)
  );
}
...
function isJSDocNamespaceBody(node) {
  return (
    ts9__default.default.isIdentifier(node) || isJSDocNamespaceDeclaration(node)
  );
}
...
function isJsxAttributeLike(node) {
  return (
    ts9__default.default.isJsxAttribute(node) ||
    ts9__default.default.isJsxSpreadAttribute(node)
  );
}
...
function isModuleReference(node) {
  return (
    ts9__default.default.isEntityName(node) ||
    ts9__default.default.isExternalModuleReference(node)
  );
}
function isNamedImportBindings(node) {
  return (
    ts9__default.default.isNamespaceImport(node) ||
    ts9__default.default.isNamedImports(node)
  );
}
function isNamedImportsOrExports(node) {
  return (
    ts9__default.default.isNamedImports(node) ||
    ts9__default.default.isNamedExports(node)
  );
}
function isNamespaceBody(node) {
  return (
    ts9__default.default.isModuleBlock(node) || isNamespaceDeclaration(node)
  );
}
...
function isTypeOnlyCompatibleAliasDeclaration(node) {
  ...
  if (
    isTsVersionAtLeast(5, 0) &&
    (ts9__default.default.isExportDeclaration(node) ||
      ts9__default.default.isNamespaceExport(node))
  ) {
    return true;
  }
  return false;
...
function isUnionOrIntersectionTypeNode(node) {
  return (
    ts9__default.default.isUnionTypeNode(node) ||
    ts9__default.default.isIntersectionTypeNode(node)
  );
}
...
function isSuperElementAccessExpression(node) {
  return (
    ts9__default.default.isElementAccessExpression(node) &&
    isSuperExpression(node.expression)
  );
}
...
function isUnionOrIntersectionType(type) {
  return isTypeFlagSet(
    type,
    ts9__default.default.TypeFlags.UnionOrIntersection
  );
}
...
function isEvolvingArrayType(type) {
  return (
    isObjectType(type) &&
    isObjectFlagSet(type, ts9__default.default.ObjectFlags.EvolvingArray)
  );
}
function isTupleType(type) {
  return (
    isObjectType(type) &&
    isObjectFlagSet(type, ts9__default.default.ObjectFlags.Tuple)
  );
}
function isTypeReference(type) {
  return (
    isObjectType(type) &&
    isObjectFlagSet(type, ts9__default.default.ObjectFlags.Reference)
  );
}
    ...
    if (getPropertyOfType(subType, name) === undefined) {
      const index =
        (isNumericPropertyName(name)
          ? typeChecker.getIndexInfoOfType(
              subType,
              ts9__default.default.IndexKind.Number
            )
          : undefined) ??
        typeChecker.getIndexInfoOfType(
          subType,
          ts9__default.default.IndexKind.String
        );
      if (index?.isReadonly) {
        if (seenProperty) {
          return true;
...
function isReadonlyPropertyFromMappedType(type, name, typeChecker) {
  if (
    !isObjectType(type) ||
    !isObjectFlagSet(type, ts9__default.default.ObjectFlags.Mapped)
  ) {
    return;
  }
  ...
  if (declaration.readonlyToken !== undefined && !/^__@[^@]+$/.test(name)) {
    return (
      declaration.readonlyToken.kind !==
      ts9__default.default.SyntaxKind.MinusToken
    );
  }
...
function getDeclarationDomain(node) {
  switch (node.parent.kind) {
    ...
    case ts9__default.default.SyntaxKind.Parameter:
      if (
        node.parent.parent.kind ===
          ts9__default.default.SyntaxKind.IndexSignature ||
        identifierToKeywordKind(node) ===
...
function getPropertyName(propertyName) {
  if (
    propertyName.kind === ts9__default.default.SyntaxKind.ComputedPropertyName
  ) {
...
function unwrapParentheses(node) {
  while (
    node.kind === ts9__default.default.SyntaxKind.ParenthesizedExpression
  ) {
...
function getUsageDomain(node) {
  ...
  switch (parent.kind) {
    ...
    case ts9__default.default.SyntaxKind.ExpressionWithTypeArguments:
      return parent.parent.token ===
        ts9__default.default.SyntaxKind.ImplementsKeyword ||
        parent.parent.parent.kind ===
          ts9__default.default.SyntaxKind.InterfaceDeclaration
        ? 2 /* Type */
        : 4 /* Value */;
    case ts9__default.default.SyntaxKind.QualifiedName:
      if (parent.left === node) {
        if (
          getEntityNameParent(parent).kind ===
          ts9__default.default.SyntaxKind.TypeQuery
        ) {
    ...
    case ts9__default.default.SyntaxKind.TypeReference:
      return identifierToKeywordKind(node) !==
        ts9__default.default.SyntaxKind.ConstKeyword
        ? 2 /* Type */
        : undefined;
    default:
      return 5 /* ValueOrNamespace */;
...
var UsageWalker = class {
  ...
  getUsage(sourceFile) {
    ...
    const cb = node => {
      ...
      switch (node.kind) {
        ...
        case ts9__default.default.SyntaxKind.ExportAssignment:
          if (
            node.expression.kind === ts9__default.default.SyntaxKind.Identifier
          ) {
  ...
  #handleFunctionLikeDeclaration(node, cb, varCb) {
    ...
    const scope = (this.#scope =
      node.kind === ts9__default.default.SyntaxKind.FunctionExpression &&
      node.name !== undefined
  ...
  #handleVariableDeclaration(declarationList) {
    ...
    const exported =
      declarationList.parent.kind ===
        ts9__default.default.SyntaxKind.VariableStatement &&
      includesModifier(
...
function isBlockScopedVariableDeclarationList(declarationList) {
  return (
    (declarationList.flags & ts9__default.default.NodeFlags.BlockScoped) !== 0
  );
}
...
function namespaceHasExportStatement(ns) {
  if (
    ns.body === undefined ||
    ns.body.kind !== ts9__default.default.SyntaxKind.ModuleBlock
  ) {
    return false;
  }
...
exports.isDeclarationWithTypeParameterChildren =
...
exports.isPropertyAccessEntityNameExpression =
...
exports.isTypeOnlyCompatibleAliasDeclaration =
